...
class Configuration extends AbstractModule with ScalaModule {
  ...
  override final def configure() {
    ...
    val textMaps = Map(
      "sourceUrl" -> Properties.envOrElse("PULL_BASE_URL", "http://localhost/offenders/casenotes"),
      "targetUrl" -> Properties.envOrElse("PUSH_BASE_URL", "http://localhost/api"),
      "username" -> Properties.envOrElse("PUSH_USERNAME", "username"),
      "password" -> Properties.envOrElse("PUSH_PASSWORD", "password")
    )

    val numberMaps = Map(
      "seconds" -> Properties.envOrElse("POLL_SECONDS", "5").toInt  //@TODO: Read from resource file?
    )
